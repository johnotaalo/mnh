<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title><?php echo $title ?></title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Online Data Management Tool">
    <meta name="author" content="Ministry of Health, Government of Kenya">

    <!--styles -->
    <link href="<?php echo base_url(); ?>css/bootstrap.css" rel="stylesheet">
	<link href="<?php echo base_url(); ?>css/fluid-layout.css" rel="stylesheet">
	
	 <!--fav and touch icons -->
    <link rel="shortcut icon"  href="<?php echo base_url(); ?>/images/favicon.ico">
   
  </head>
  <body>
    <div class="navbar navbar-fixed-top">
      <div class="navbar-inner">
        <div class="container-fluid">
          <a class="btn btn-navbar" data-toggle="collapse" data-target=".nav-collapse">
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </a>
          
          <div class="nav-collapse">
            
            <p class="navbar-text pull-right">Logged in as <a href="#">username</a></p>
          </div><!--/.nav-collapse -->
        </div>
      </div>
    </div>
  
      		
      	<div class="col1">
        <div class="span3">
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
              <li class="nav-header">Indicators</li>
              <li class="active"><a href="#">Guideline Awareness</a></li>
              <li><a href="#">Guideline Availability</a></li>
              <li><a href="#">Signal Function</a></li>
              <li><a href="#">BemONC</a></li>
              
			 
           </ul>
          </div><!--/.well slide nav bar -->
        </div><!--/span-->
      </div><!--/.column-->
      
      
       <div class="col2">
      <div class="row" id="reponse_county_summary">
      
        <div class="span7" id="response_rate">
        <div class="selector">
    
          <h2 class="charttiltle"> Response Rate</h2>
          </p></p>
           fegwsj</br>
           asf</br>
           sd</br>
           dsf</br>
           sdx</br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           d </br>
           d </br>
           df </br>
           d </br>
        </div>
        
        </div> <!--/.response_rate-->    
   <div class="container-fluid">&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
        <div class="span7" id="county_summary" >
        
       <div class="selector">
      
          <h2 class="charttiltle"> County Summary</h2>
          <p></p>
          
           fegwsj</br>
           asf</br>
           sd</br>
           dsf</br>
           sdx</br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           d </br>
           d </br>
           df </br>
           d </br>
          
       </div>
        
      </div><!--/.county_summary-->
    </div><!--/.reponse_county_summary-->
    
  
    
    </div><!--/.column-->
     
     <div class="col3">
      <div class="row"><!--begin row 2-->
      
        <div class="span7" id="district_summary">
        <div class="selector">
     
          <h2 class="charttiltle" style="">District Summary</h2>
          </p></p>
          
           fegwsj</br>
           asf</br>
           sd</br>
           dsf</br>
           sdx</br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           d </br>
           d </br>
           df </br>
           d </br>
          
        </div>
                </div><!--./district_summary-->  
                <div class="container-fluid">&nbsp;&nbsp;&nbsp;&nbsp;
</div> 
    
        <div class="span7" id="commodity_summary" >
        
       <div class="selector">
      
          <h2 class="charttiltle" style=""> Commodity Summary</h2>
          <p></p>
          
           fegwsj</br>
           asf</br>
           sd</br>
           dsf</br>
           sdx</br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           sd </br>
           d </br>
           d </br>
           df </br>
           d </br>
          
       </div>
        
      </div><!--./commodity_summary-->
   </div> <!--row 2-->
   </div><!--/.column-->
 
  
 
 
 </div><!--/.fluid-container-->
     </div><!--./center-content-->  

    
    
    <!-- Le javascript
    ================================================== -->
    <!-- Placed at the end of the document so the pages load faster -->
    <script src="http://code.jquery.com/jquery-1.9.1.js"></script>
    <script src="<?php echo base_url(); ?>js/bootstrap/bootstrap-combined.js"></script>
    <script src="<?php echo base_url();?>js/highcharts.js" type="text/javascript"></script>
	<script src="http://code.highcharts.com/modules/exporting.js"></script>
    


  </body>
</html>