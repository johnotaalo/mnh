...
ORDER BY lq.lq_response ASC";
    ...
    */
    ...
    public function getTreatmentStatistics($criteria, $value, $survey, $survey_category) {
        $value = urldecode($value);
        $query = "CALL get_treatment_statistics('" . $criteria . "', '" . $value . "', '" . $survey . "','" . $survey_category . "');";
        try {
            $queryData = $this->db->query($query, array($value));
            $this->dataSet = $queryData->result_array();
            $queryData->next_result();
...
ORDER BY question_code";
     ...
     * Beds in facility
     */
    public function getBeds($criteria, $value, $survey, $survey_category) {
    ...
    public function getSuppliesLocation($criteria, $value, $survey, $survey_category, $for) {
        ...
        try {
            ...
            foreach ($this->dataSet as $value_) {
                ...
                //echo "<pre>";print_r($this->dataSet);echo "</pre>";die;
                $question = $this->getSupplyName($value_['supplies'], $survey);
                
                $mch = $value_['phar_values'];
                $ward = $value_['sto_values'];
                $opd = $value_['del_values'];
                $uc = $value_['ot_values'];
                
                
                //1. collect the categories
                $data[$question]['pharmacy'] = $mch;
                $data[$question]['store'] = $ward;
                $data[$question]['delivery room'] = $opd;
                $data[$question]['other'] = $other;
            }
