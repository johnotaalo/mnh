...
<tr id="transfusion_n">
    ...
    public function createMCHStaffTrainingGuidelinesSection() {
        ...
        $clinicalStaff = array('Doctor', 'Nurse', 'R.C.O.');
        $titles = array('Total in Facility', 'Total Available On Duty');
        $subTitles = array('Before 2010', 'After 2010');
        
        //var_dump($this->data_found);die;
        $counter = 0;
        foreach ($this->data_found as $value) {
            $titles[] = 'Number of Staff Trained In ' . $value['guideName'];
        }
        $titles[] = 'Still Working';
        
        //echo '<pre>'; print_r($titles); echo '</pre>';die;
        
        foreach ($clinicalStaff as $staff) {
            $beginning[] ='<td width="50px">'.$staff.'</td>';
            for ($x = 0; $x < sizeof($titles); $x++) {
                $fields = '<td width="50px"><input style="width:100px" type="text" style="width:50px" name="' . str_replace(' ', '_', $titles[$x]) . '"</td>';
                foreach ($this->data_found as $value) {
                    $name = $value['guideName'];
                    $arr = explode(' ', trim($name));
                    $fields = (strpos($titles[$x], $value['guideName']) != false) ? '<td><input type="text" style="width:50px" name="before2010_' . $arr[0] . '"></td><td><input type="text" style="width:50px" name="after2010_' . $arr[0] . '"></td>' : $fields;
                   
                }
                $finalDataSet[$staff][] = $fields;
            }
        }
       $counter=0;
       //echo '<pre>';print_r($finalDataSet);echo '</pre>';die;
        foreach ($finalDataSet as $key => $value) {
            
            $this->mchTrainingGuidelineSection[$key] = '<tr><td>'.$key.'</td>';
            $counter++;
            foreach ($value as $val) {
                $this->mchTrainingGuidelineSection[$key].= $val;
            }
            $this->mchTrainingGuidelineSection[$key].= '</tr>';
        }
        //echo '<table>'.$this->mchTrainingGuidelineSection['Doctor'].$this->mchTrainingGuidelineSection['Nurse'].'</table>';die;
    ...
    public function createTreatmentsMCHSection() {
        ...
        $counter = 0;
        foreach ($newData as $theData) {
            foreach ($theData as $k => $value) {
                $counter++;
                
                //echo '<pre>';print_r($value);echo '</pre>';die;
                $other = '<tr><td>Other(specify)</td>  ';
                $normal = ' <tr><td>' . $k . '</td>';
                
                foreach ($value as $treat) {
                    
                    //echo '<pre>';print_r($treat);echo '</pre>';die;
                    $normal.= '<td style ="text-align:center;">
            <input name="mch_' . str_replace(' ', '_', $treat['tcName']) . '_' . $counter . '" type="text"  size="8" />
            </td>';
                    $other.= '<td style ="text-align:center;">
            <input name="mch_' . $treat['tcName'] . '_Other_' . $counter . '" type="text"  size="8" />
            </td>';
                }
                $other.= '</td>
            <input type="hidden"  name="mchtTreatmentCode_' . $treat['tcName'] . '_Other_' . $counter . '" id="mchtTreatmentCode_' . $counter . '" value="' . $treat['treatmentCode'] . '" />
        </tr>';
                $normal.= '</td>
            <input type="hidden"  name="mchtTreatmentCode_' . $counter . '" id="mchtTreatmentCode_' . $counter . '" value="' . $treat['treatmentCode'] . '" />
        </tr>';
                
                $data[$value[0]['treatmentFor']][] = ($k == 'Others') ? $other : $normal;
            }
        }
        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($value as $val) {
                $this->treatmentMCHSection[$key].= $val;
            }
            $this->treatmentMCHSection[$key].= '</tr>';
        }
        }
